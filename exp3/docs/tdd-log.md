...
- **Further RED `614d792`:** Added `shouldSupportDecimalPriceUpdates`, the seventh additional preimplementation case. `decimal-update-compile-red.txt` records a Maven test-compilation failure: a `double` argument could not be converted to the stub's `int`. Thus **3 supplied + 7 additional = 10 cases** preceded implementation. A separate exploratory decimal report is designated for private archival outside the repository; it is not committed RED history or submission evidence.
