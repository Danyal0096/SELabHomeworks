...
| `final-verification/` | `jacoco.xml` and `mutations.xml` originally retained in `41e4e87` and refreshed after the CX-07 validation tests; the latter records 28 killed of 28 generated `ShoppingCart` mutations. |

Local `target/surefire-reports/` currently totals 43 passing tests, but no committed final clean-checkout console log or exit-code file is present. The empty placeholder directories under `evidence/` contain no additional measurements. Preserve the distinction between saved reports, local generated output, and owner-supplied observations.
